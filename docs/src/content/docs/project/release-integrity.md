...
```bash
gh release download v1.0.2 --repo udid-tools/device-info --dir device-info-release
...
```bash
sha256sum --check udid-tools-device-info-1.0.2.tgz.sha256
```

On macOS, use the compatible checksum command:

```bash
shasum --algorithm 256 --check udid-tools-device-info-1.0.2.tgz.sha256
```

Verify the tarball's keyless Sigstore signature, certificate identity, and transparency-log proof:

```bash
cosign verify-blob udid-tools-device-info-1.0.2.tgz \
  --bundle udid-tools-device-info-1.0.2.tgz.sigstore.json \
  --certificate-identity \
  "https://github.com/udid-tools/device-info/.github/workflows/release.yml@refs/tags/v1.0.2" \
...
```bash
gh attestation verify udid-tools-device-info-1.0.2.tgz \
